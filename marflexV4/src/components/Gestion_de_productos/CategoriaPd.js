...
const Categorias = () => {
  ...
  const agregarCategoria = () => {
    axios
      .post("http://localhost:3000/api/api/agregar/categoriasPd", formularioDatos)
      .then(() => {
        obtenerCategorias();
        cerrarFormulario();
  ...
  const editarCategoria = () => {
    axios
      .put(
        `http://localhost:3000/api/api/editar/categoriasPd/${formularioDatos.ID}`,
        formularioDatos
      )
      .then(() => {
        obtenerCategorias();
  ...
  const eliminarCategoria = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:3000/api/api/eliminar/categoriasPd/${id}`)
          .then(() => {
            obtenerCategorias();
