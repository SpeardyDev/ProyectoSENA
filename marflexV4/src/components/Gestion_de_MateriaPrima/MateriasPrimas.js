...
const MateriasPrimas = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/api/api/proveedores')
  ...
  const mostrarProductos = () => {
    axios.get('http://localhost:3000/api/api/materiasprimas')
  ...
  const AgregarMateriaPrima = (e) => {
    ...
    if (editandoID) {
      axios
        .put(`http://localhost:3000/api/api/materiasprimas/editar/${editandoID}`, formularioDatos)
        .then((response) => {
      ...
      axios
        .post("http://localhost:3000/api/api/materiasprimas/agregar", formularioDatos)
        .then((response) => {
          console.log(response.data);
          setMostrarFormulario(false);
          mostrarProductos();
  ...
  const Eliminar = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`http://localhost:3000/api/api/materiasprimas/borrar/${id}`)
          .then(response => {
            console.log(response.data);
            mostrarProductos();
            Swal.fire({
          ...
          <div className='contenedor_formulario_Proveedores'>
            ...
            <FormGroup widths='equal'>
              <FormField required>
                ...
                <Select placeholder='Selecciona Proveedor' onChange={handleSelectChange} name='Proveedor' value={formularioDatos.Proveedor} options={Proveedores.map(Proveedor => ({ key: Proveedor.ID, value: Proveedor.ID, text: Proveedor.Nombre }))} />
              </FormField>
            </FormGroup>
        ...
        <TableBody>
          {currentItems.map((producto, index) => (
            <TableRow key={producto.ID}>
              ...
              <TableCell>
                {Proveedores.find(
                  (Proveedor) => Proveedor.ID === producto.Proveedor
                )?.Nombre || "Desconocido"}
              </TableCell>
              <TableCell>
