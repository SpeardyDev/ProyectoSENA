...
useEffect(() => { 
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:3000/api/agregar/productos', formularioDatos)
      .then(response => {
  ...
  return (
   <div className="contenedor-Productos">
    <div className='titulo'><p>Productos</p></div>
    {mostrarFormulario && (
        <Form className='RegistroNuevoProveedor' onSubmit={handleSubmit}>
          <div className='contenedor_formulario_Proveedores'>
            <FormGroup widths='equal'>
             ...
             <FormField required>
               <label>Precio</label>
               <InputNumber onChange={(e) => handleInputNumberChange('Precio', e.value)} name="Precio" value={formularioDatos.Precio} placeholder="Precio"/>
             </FormField>
            ...
            <FormGroup widths='equal'>
             ...
             <FormField required>
             <label>Estado</label>
             <Select placeholder='Estado' onChange={handleSelectChange} name='EstadoID' value={formularioDatos.EstadoID} options={estados.map( estados=> ({key: estados.ID,  value: estados.ID, text: estados.Estado}))} />
             </FormField>
            ...
            <FormGroup widths='equal'>
              ...
              <Calendar value={formularioDatos.FechaIngreso} onChange={handleChange} name='FechaIngreso' placeholder='00/00/0000' required/>
              </FormField>
              
              <FormField required>
                <label>Proveedor</label>
              <Select placeholder='colchon' onChange={handleSelectChange} name='ProveedorID' value={formularioDatos.ProveedorID} options={Proveedores.map( Proveedores=> ({key: Proveedores.ID,  value: Proveedores.ID, text: Proveedores.Nombre}))} />
              </FormField>
    ...
    <Button onClick={() => setMostrarFormulario(!mostrarFormulario)} color='green'><i className="pi pi-plus"/> Producto</Button>
      </div>
    </div>
    ...
    <TableHeader>
      ...
      <TableHeaderCell>#</TableHeaderCell>
        ...
        <TableHeaderCell>Proveedor ID</TableHeaderCell>
        <TableHeaderCell></TableHeaderCell>
      </TableRow>
    </TableHeader>
    ...
    <TableBody>
       {currentItems.map((producto, index) => (
          <TableRow key={producto.ID}>
            ...
            <TableCell>{producto.ProveedorID}</TableCell>
            <TableCell>
            <button type="button" className="btn btn-primary"><FontAwesomeIcon className='Icon' icon={faPenToSquare} /> Editar</button>
            <button type="button" className="btn btn-danger" onClick={() => handleDelete(producto.ID)}><FontAwesomeIcon className='Icon' icon={faTrashCan} /> Eliminar</button>
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
